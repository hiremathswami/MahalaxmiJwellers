...
import OwnerSidebar from '@/components/panels/OwnerSidebar';
import TopBar from '@/components/panels/TopBar';
import { useOwnerGuard } from '@/lib/roleGuard';

export default function OwnerLayout({ children }) {
  const { authorized, loading } = useOwnerGuard();
  const [title, setTitle] = useState('Owner Panel');
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center font-inter">
        <div className="space-y-4 text-center">
          ...
          <p className="text-[9px] uppercase tracking-[0.2em] text-gray-400 font-bold">Verifying Owner Access...</p>
        </div>
      </div>
    );
  }
